...
function CommentForm({ postId }) {
    ...
    const doSubmit = async (data) => {
        ...
        await appwritedatabase.addComment(postId, [
            ...comment,
    ...
    useEffect(() => {
        appwritedatabase.getComment(postId).then((data) => {
            if (data.comment.join('').trim().length != 0)
    ...
    return (
        <>
            <div className="mt-10">
                ...
                <form onSubmit={handleSubmit(doSubmit)}>
                    <div className="flex flex-row max-w-[40rem]">
                        <Input
                            type="text"
                            className="w-full h-full px-4 py-1 outline-none border border-black"
                            {...register('comment', {
                                required: true,
                            })}
                        />
            ...
            {comment.length > 0 ? (
                ...
                <div className="text-gray-800 mt-5">No Comments</div>
            )}
        </>
    )
}
