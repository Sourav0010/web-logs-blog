...
function User() {
   ...
   const update = (data) => {
      ...
      if (data.email !== userData.email) {
         appwriteAuth.updateEmail({
            email: data.email,
            password: getValues('password'),
         });
      }
      if (data.phone !== userData.phone) {
         appwriteAuth.updatePhone({
            phone: data.phone,
            password: getValues('password'),
         });
      }
      ...
      <div className='w-full flex flex-col items-center justify-center gap-5 my-4'>
         ...
         <form className='flex gap-4 flex-col' onSubmit={handleSubmit(update)}>
            ...
            <Input
               type='text'
               lebel='Email'
               className='bg-white border p-2  font-normal'
               placeholder='Name'
               {...register('email')}
            />
            <Input
            ...
            <Button
               type='submit'
               className='bg-black hover:cursor-pointer text-white p-2 font-normal rounded-md'
            >
               Update
            </Button>
         </form>
